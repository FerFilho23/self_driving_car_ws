...
def estimate_midlane(midlane_patches,max_dist):
    ...
    # Keep a Midlane_draw for displaying shortest connectivity later on
    ...
    # Connect each contous with its closest 
...
def ApproxDistBWCntrs(cnt, cnt_cmp):
    ...
    M = cv2.moments(cnt)
    if M["m00"] != 0:  # Ensure the area (m00) is not zero
        cX = int(M["m10"] / M["m00"])
        cY = int(M["m01"] / M["m00"])
    else:
        cX, cY = 0, 0  # Default values when the area is zero

    # Compute the center of the contour
    M_cmp = cv2.moments(cnt_cmp)
    if M_cmp["m00"] != 0:  # Ensure the area (m00) is not zero
        cX_cmp = int(M_cmp["m10"] / M_cmp["m00"])
        cY_cmp = int(M_cmp["m01"] / M_cmp["m00"])
    else:
        cX_cmp, cY_cmp = 0, 0  # Default values when the area is zero
